...
template sudoku() {
    ...
    component puzz_RangeProofs[9][9];

    component sol_RangeProofs[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzz_RangeProofs[i][j] = RangeProof(32);
            sol_RangeProofs[i][j] = RangeProof(32);

            puzz_RangeProofs[i][j].in <== puzzle[i][j];
            puzz_RangeProofs[i][j].range[0] <== 0;
            puzz_RangeProofs[i][j].range[1] <== 9;

            sol_RangeProofs[i][j].in <== solution[i][j];
            sol_RangeProofs[i][j].range[0] <== 0;
            sol_RangeProofs[i][j].range[1] <== 9;

            assert(puzz_RangeProofs[i][j].out == 1);
            assert(sol_RangeProofs[i][j].out == 1);
